...
func (env LDKLightningNetworkEnv) WaitForBobSynced(ctx context.Context, timeout time.Duration) error {
	if env.BobLnd == nil {
		return fmt.Errorf("bob lnd container is nil")
	}

	deadline := time.Now().Add(timeout)
	var lastInfo ldkLndInfo
	var lastErr error
	for time.Now().Before(deadline) {
		info, err := ldkBobInfo(ctx, env.BobLnd)
		if err != nil {
			lastErr = err
	...
	return fmt.Errorf("timed out waiting for bob to sync to chain: last_info=%+v last_err=%v", lastInfo, lastErr)
}

func (env LDKLightningNetworkEnv) WaitForBobPendingChannel(ctx context.Context, timeout time.Duration) error {
	if env.BobLnd == nil {
		return fmt.Errorf("bob lnd container is nil")
	}
	...
	for time.Now().Before(deadline) {
		...
		lastInfo = info
		if info.NumPendingChannels > 0 || info.NumActiveChannels > 0 {
			return nil
		}
		time.Sleep(500 * time.Millisecond)
	}

	return fmt.Errorf("timed out waiting for bob to observe a channel open: last_info=%+v last_err=%v", lastInfo, lastErr)
}
...
func (env LDKLightningNetworkEnv) WaitForBobOutbound(ctx context.Context, minLocalBalanceSat uint64, timeout time.Duration) error {
	...
	return fmt.Errorf("timed out waiting for bob outbound balance >= %d, last_info=%+v last_channels=%+v last_err=%v", minLocalBalanceSat, lastInfo, lastResp.Channels, lastErr)
}
