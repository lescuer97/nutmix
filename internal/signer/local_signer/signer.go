...
func (l *LocalSigner) GetKeysets() (signer.GetKeysetsResponse, error) {
	...
	for _, seed := range seeds {
		if seed.Unit != cashu.AUTH.String() {
			response.Keysets = append(response.Keysets, cashu.BasicKeysetResponse{Id: seed.Id, Unit: seed.Unit, Active: seed.Active, InputFeePpk: seed.InputFeePpk, Version: seed.Version})
		}
	}
	return response, nil
}
...
func (l *LocalSigner) createNewSeed(mintPrivateKey *bip32.Key, unit cashu.Unit, version uint32, fee uint) (cashu.Seed, error) {
	// rotate one level up
	newSeed := cashu.Seed{
