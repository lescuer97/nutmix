...
func TestCreateNewSeed(t *testing.T) {
	...
	if keys.Keysets[0].Id != "00299ac0e7a8fe63" {
		t.Errorf("seed id incorrect. %v", keys.Keysets[0].Id)
	}
}
...
func TestDeriveKeysetSat(t *testing.T) {
	...
	for i := range mintKeyset {
		val, ok := keysResult[strconv.FormatUint(mintKeyset[i].Amount, 10)]
		if !ok {
			t.Fatalf("There should always be key for value")
		}
		justPubkeys = append(justPubkeys, mintKeyset[i].GetPubKey())
		if hex.EncodeToString(mintKeyset[i].PrivKey.PubKey().SerializeCompressed()) != val {
			t.Errorf("key values should be the same: \n CalculatedKey %x. \n NeededKey %v", mintKeyset[i].PrivKey.PubKey().SerializeCompressed(), val)
		}
	}
	id, err := DeriveKeysetId(justPubkeys)
	if err != nil {
		t.Fatalf("DeriveKeysetId(justPubkeys) %v", err)
	}

	if id != "00b5a0580f75cc2f" {
...
func TestLoadLegacyAndNonLegacySeeds(t *testing.T) {
	...
	legacyKey, err := bip32.NewMasterKey(privateKeyBytes)
	if err != nil {
		t.Fatalf("could not setup legacy master key")
	}
	masterKey, err := hdkeychain.NewMaster(privateKeyBytes, &chaincfg.MainNetParams)
	if err != nil {
		t.Fatalf("could not setup master key")
	...
	// Create Legacy Seed
	legacySeed := cashu.Seed{
		Active:         true,
		CreatedAt:      time.Now().Unix(),
		Version:        0,
		Unit:           cashu.Sat.String(),
		DerivationPath: "0/0/0", // Legacy path
		InputFeePpk:    0,
		Amounts:        cashu.GetAmountsForKeysets(cashu.LegacyMaxKeysetAmount),
		Legacy:         true,
	}

	// Calculate correct ID for legacy seed
	legacyKeysets, err := legacyDeriveKeyset(legacyKey, legacySeed)
	if err != nil {
	...
	legacyId, err := DeriveKeysetId(legacyPubkeys)
	if err != nil {
		t.Fatalf("DeriveKeysetId(legacy) %v", err)
	}
	legacySeed.Id = legacyId

	// Create Non-Legacy Seed
	nonLegacySeed := cashu.Seed{
		Active:         true,
		CreatedAt:      time.Now().Unix(),
		Version:        1,
		Unit:           cashu.Sat.String(),
	...
	// Calculate correct ID for non-legacy seed
	nonLegacyKeysets, err := DeriveKeyset(masterKey, nonLegacySeed)
	if err != nil {
	...
	nonLegacyId, err := DeriveKeysetId(nonLegacyPubkeys)
	if err != nil {
		t.Fatalf("DeriveKeysetId(nonLegacy) %v", err)
	}
	nonLegacySeed.Id = nonLegacyId

	// Save to DB
	err = db.SaveNewSeeds([]cashu.Seed{legacySeed, nonLegacySeed})
	if err != nil {
		t.Fatalf("db.SaveNewSeeds %v", err)
	}
	...
	// Verify Results
	keysetsResponse, err := localsigner.GetKeysets()
	if err != nil {
		t.Fatalf("localsigner.GetKeysets() %+v", err)
	}

	if len(keysetsResponse.Keysets) != 2 {
		t.Errorf("Expected 2 keysets, got %d", len(keysetsResponse.Keysets))
	}

	// Verify IDs match
	foundLegacy := false
	foundNonLegacy := false
	for _, k := range keysetsResponse.Keysets {
		if k.Id == legacyId {
			foundLegacy = true
		}
		if k.Id == nonLegacyId {
	...
	if !foundLegacy {
		t.Errorf("Legacy keyset ID %s not found in response", legacyId)
	}
	if !foundNonLegacy {
		t.Errorf("Non-Legacy keyset ID %s not found in response", nonLegacyId)
	}
}
