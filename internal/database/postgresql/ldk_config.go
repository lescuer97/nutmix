...
func (pql Postgresql) GetLDKConfig(ctx context.Context) (database.LDKConfig, error) {
	...
	err := pql.pool.QueryRow(ctx, `
		SELECT chain_source_type, electrum_server_url, rpc_address, rpc_username, rpc_password, rpc_port, config_directory
		FROM ldk
		WHERE id = 1
	`).Scan(&config.ChainSourceType, &config.ElectrumServerURL, &config.Rpc.Address, &config.Rpc.Username, &config.Rpc.Password, &config.Rpc.Port, &config.ConfigDirectory)
	if err != nil {
...
func (pql Postgresql) SetLDKConfig(ctx context.Context, config database.LDKConfig) error {
	_, err := pql.pool.Exec(ctx, `
		INSERT INTO ldk (id, chain_source_type, electrum_server_url, rpc_address, rpc_username, rpc_password, rpc_port, config_directory)
	...
	`, config.ChainSourceType, config.ElectrumServerURL, config.Rpc.Address, config.Rpc.Username, config.Rpc.Password, config.Rpc.Port, config.ConfigDirectory)
	if err != nil {
