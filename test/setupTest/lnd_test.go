...
func TestSetupLightingCommsLND(t *testing.T) {
	...
	invoice, err := lndWallet.RequestInvoice(cashu.Amount{Amount: 1000, Unit: cashu.Sat})
	if err != nil {
...
func TestSetupLightingCommsLnBits(t *testing.T) {
	...
	invoice, err := lnbitsWallet.RequestInvoice(cashu.Amount{Amount: 1000, Unit: cashu.Sat})
	if err != nil {
		t.Fatalf("could not setup lighting comms %+v", err)
	}
