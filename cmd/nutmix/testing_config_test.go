...
func applyTestingConfigEnv(config *utils.Config) error {
	backend, err := utils.StringToLightningBackend(os.Getenv(mint.MINT_LIGHTNING_BACKEND_ENV))
	if err != nil {
		return err
	}
	config.MINT_LIGHTNING_BACKEND = backend
