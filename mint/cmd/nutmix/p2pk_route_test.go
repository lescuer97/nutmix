...
func TestRoutesSwapMelt(t *testing.T) {
	...
	swapProofs, err := GenerateProofsP2PK(postMintResponse.Signatures, mint.ActiveKeysets, p2pkMintingSecrets, P2PKMintingSecretKeys, lockingPrivKey)

	if err != nil {
		t.Fatalf("Error generating proofs: %v", err)
	}

	swapBlindedMessagesP2PK, swapSecretsP2PK, swapSecretKeyP2PK, err := CreateP2PKBlindedMessages(1000, mint.ActiveKeysets[cashu.Sat.String()][1], lockingPrivKey.PubKey(), 1, []*secp256k1.PublicKey{lockingPrivKey.PubKey()}, nil, 0, cashu.SigAll)
	...
	swapProofsWrongSigs, err := GenerateProofsP2PK(postMintResponse.Signatures, mint.ActiveKeysets, swapSecretsP2PK, swapSecretKeyP2PK, wrongPrivKey)
...
func CreateP2PKBlindedMessages(amount uint64, keyset cashu.Keyset, pubkey *secp256k1.PublicKey, nSigs int, pubkeys []*secp256k1.PublicKey, recoverpubKey []*secp256k1.PublicKey, locktime int, sigflag cashu.SigFlag) ([]cashu.BlindedMessage, []string, []*secp256k1.PrivateKey, error) {
	...
	for i, amt := range splitAmounts {
		spendCond, err := makeP2PKSpendCondition(pubkey, nSigs, pubkeys, recoverpubKey, locktime, sigflag)
...
func makeP2PKSpendCondition(pubkey *secp256k1.PublicKey, nSigs int, pubkeys []*secp256k1.PublicKey, recoverpubKey []*secp256k1.PublicKey, locktime int, sigflag cashu.SigFlag) (cashu.SpendCondition, error) {
	...
	spendCondition.Data.Tags.Refund = recoverpubKey
...
func GenerateProofsP2PK(signatures []cashu.BlindSignature, keysets map[string]KeysetMap, secrets []string, secretsKey []*secp256k1.PrivateKey, privkey *secp256k1.PrivateKey) ([]cashu.Proof, error) {
	// try to swap tokens
